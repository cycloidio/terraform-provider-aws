...
const (
	packageName = "simpledb"
	awsEnvVar   = "AWS_ENDPOINT_URL_SIMPLEDB"
	baseEnvVar  = "AWS_ENDPOINT_URL"
	configParam = "simpledb"
...
func defaultEndpoint(region string) (url.URL, error) {
	...
	ep, err := r.EndpointFor(simpledb_sdkv1.EndpointsID, region)
	if err != nil {
		return url.URL{}, err
	}
...
func defaultFIPSEndpoint(region string) (url.URL, error) {
	...
	ep, err := r.EndpointFor(simpledb_sdkv1.EndpointsID, region, func(opt *endpoints.Options) {
...
func callService(ctx context.Context, t *testing.T, meta *conns.AWSClient) apiCallParams {
	...
	client := meta.SimpleDBConn(ctx)

	req, _ := client.ListDomainsRequest(&simpledb_sdkv1.ListDomainsInput{})
